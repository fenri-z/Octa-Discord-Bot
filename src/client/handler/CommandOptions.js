 ...
 * @param {boolean} isDM - apakah interaction terjadi di DM
 * @returns {boolean}
 */
const handleApplicationCommandOptions = async (interaction, options, command, isDM = false) => {

    if (options.devGuildOnly) {
        if (interaction.guild?.id !== config.development.devGuildId) {
            await interaction.reply({
                content: config.messages.NOT_DEV_GUILD,
    ...
    // guildOwner tidak berlaku di DM
    if (options.guildOwner && !isDM) {
 ...
 * @param {boolean} isDM - apakah message dikirim di DM
 * @returns {boolean}
 */
const handleMessageCommandOptions = async (message, options, command, isDM = false) => {

    if (options.devGuildOnly) {
        if (message.guild?.id !== config.development.devGuildId) {
            await message.reply({ content: config.messages.NOT_DEV_GUILD });
            return false;
        }
    }
    ...
    if (options.botOwner) {
        if (message.author.id !== config.users.ownerId) {
            await message.reply({ content: config.messages.NOT_BOT_OWNER });
            return false;
        }
    }
    ...
    if (options.botDevelopers) {
        if (config.users?.developers?.length > 0 && !config.users?.developers?.includes(message.author.id)) {
            await message.reply({ content: config.messages.NOT_BOT_DEVELOPER });
            return false;
        }
    }
    ...
    if (options.guildOwner && !isDM) {
        if (message.author.id !== message.guild?.ownerId) {
            await message.reply({ content: config.messages.NOT_GUILD_OWNER });
            return false;
        }
    }

    // nsfw tidak berlaku di DM
    if (options.nsfw && !isDM) {
        if (!message.channel.nsfw) {
            await message.reply({ content: config.messages.CHANNEL_NOT_NSFW });
            return false;
        }
    }

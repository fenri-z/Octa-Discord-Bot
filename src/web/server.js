...
async function generateStaticAssets() {
    ...
    const ytIconPath = path.join(imgDir, 'youtube.png');
    if (!fs.existsSync(ytIconPath)) {
        ...
        await sharp(svg).png().toFile(ytIconPath).catch(() => {});
        logOk('[WebServer] Generated /img/youtube.png');
    }
}
